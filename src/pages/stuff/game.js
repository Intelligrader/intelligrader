...
import { itemsData, generateInventorySlots } from './items';
import { VIEW_W, VIEW_H, TILE_SIZE, CAM_EASE, ZOOM_MIN, ZOOM_MAX, ZOOM_SPEED, TREE_COUNT, TREE_MIN_DISTANCE, TREE_SPACING, updateViewSize } from './constants';
import { clamp, lerp } from './math';
import { windowToCanvas } from './utils';
import { loadImage, loadItemSprites } from './assets';
import { checkTreeCollisions as _checkTreeCollisions } from './collisions';

import { drawWorld, drawPlayer, drawTrees, generateTreeGrid } from './draw/world';
import { HOTBAR_SLOTS, getHotbarLayout, getHotbarSlotRects, drawHotbar } from './draw/hotbar';
import {
...
export default function TwoDIn3DWorld() {
  ...
  useEffect(() => {
    ...
    function generateTrees(worldW, worldH, playerSpawnX, playerSpawnY) {
      ...
      let trees = [];
      worldImg.onload = () => {
        worldW = worldImg.width;
        worldH = worldImg.height;
        player.x = worldW / 2;
        player.y = worldH / 2;
        trees = generateTreeGrid(worldW, worldH, player); // 🌲 here’s the call
        loop();
      };
